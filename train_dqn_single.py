...
FIXED: Xử lý lỗi input dimension khi chạy 1 Agent.
"""
import numpy as np
import torch
...
class DQN(nn.Module):
    ...
    def forward(self, x):
        # Đảm bảo input là (Batch, H, W, C) hoặc (H, W, C)
        # Nếu (H, W, C) -> unsqueeze thành (1, H, W, C)
        if x.dim() == 3:
            x = x.unsqueeze(0)
            
        # Permute để chuyển thành (Batch, Channels, Height, Width) cho PyTorch Conv2d
...
class SharedAgent:
    ...
    def select_action(self, state, policy_net, epsilon, device, training=True):
        ...
        with torch.no_grad():
            ...
            # Nếu tensor là 3D (H, W, C), thêm batch dim -> (1, H, W, C)
    ...
    def train(self):
        ...
        for episode in range(self.start_episode, self.config.NUM_EPISODES + 1):
            ...
            while not all(dones) and step < self.config.MAX_STEPS_PER_EPISODE:
                ...
                # --- FIX: Đảm bảo output từ step luôn là list nếu chạy 1 snake ---
                if self.config.NUM_SNAKES == 1:
                    if not isinstance(next_obs, list):
                        next_obs = [next_obs]
                    if not isinstance(rewards, list):
                        rewards = [rewards]
                    if not isinstance(next_dones, list):
...
def evaluate(config, checkpoint_tag="best", num_episodes=10, render=True):
    ...
    for ep in range(num_episodes):
        obs = env.reset()
        if not isinstance(obs, list): obs = [obs] # FIX eval
        ...
        while not all(dones) and step < 500:
            ...
            next_obs, rewards, next_dones, _ = env.step(act_in)
            
            # FIX eval
            if config.NUM_SNAKES == 1:
                if not isinstance(next_obs, list): next_obs = [next_obs]
                if not isinstance(rewards, list): rewards = [rewards]
                if not isinstance(next_dones, list): next_dones = [next_dones]
